...
counts <- read.table(file = "/Users/paz2005/Documents/andrew_schietingerLab/rna_analysis/gene.counts.txt", header = TRUE, check.names=FALSE, row.names=1)
decoderFile <- "/Users/paz2005/Documents/andrew_schietingerLab/rna_analysis/decoder.txt"
...
counts <- counts[,c(-1,-2,-20,-21,-22,-23,-15,-16)]
decoder.data <- decoder.data[c(-1,-2,-20,-21,-22,-23,-15,-16),]
...
```{r figure2_e, warning=FALSE, cache=TRUE, echo=FALSE, comment=FALSE}
...
counts <- read.table(file = "/Users/paz2005/Documents/andrew_schietingerLab/rna_original/gene.counts.txt", header = TRUE, check.names=FALSE, row.names=1)
decoderFile <- "/Users/paz2005/Documents/andrew_schietingerLab/rna_original/decoder.txt"
decoder.data <- read.table(decoderFile,header=T,stringsAsFactors=F,sep="\t")
decoder.data$condition <- factor(decoder.data$condition)
decoder.data$group.ID <-factor(decoder.data$condition)
decoder.data <- decoder.data[decoder.data$sample.ID %in% colnames(counts),]
...
counts.sub <- counts[,grep("C", colnames(counts), invert=T)]
decoder.data.sub <- decoder.data[grep("C", decoder.data$sample.ID, invert=T),]
table( names(counts.sub) == decoder.data.sub$sample.ID)
cnts <- counts.sub
conds <- factor(make.names(decoder.data.sub$group.ID))

sampleConditions <-  factor(make.names(decoder.data.sub$group.ID))
deseq2.coldata <- data.frame(condition = sampleConditions, row.names = colnames(counts.sub))
deseq2.dds <- DESeq2::DESeqDataSetFromMatrix(countData = counts.sub, colData = deseq2.coldata, design = ~ condition)
isexpr <- rowSums(cpm(counts(deseq2.dds)) > 1) >= min(table(sampleConditions))
deseq2.dds <- deseq2.dds[isexpr,]
deseq2.dds <- DESeq(deseq2.dds)
...
```{r figure3_d, warning=FALSE, cache=TRUE, echo=FALSE, comment=FALSE}
cpms <- cpm(counts.sub[isexpr,], log=T)
my_colors = c("green", "black", "red")
my_colors = colorRampPalette(my_colors)(50)
...
```{r figure3_g, warning=FALSE, cache=TRUE, echo=FALSE, comment=FALSE}
...
cellwidth = 25
cellheight = 25
rlog_tran <- rlog(as.matrix(counts.sub))
row.names(rlog_tran) <- row.names(counts.sub)
...
deseq2_WT_vs_KO_sig <- as.data.frame(subset(results(deseq2.dds, contrast=c("condition","WT","KO"), alpha=0.05), padj < 0.05))
deseq2_WT_vs_KO_all <- as.data.frame(results(deseq2.dds, contrast=c("condition","WT","KO"), alpha=0.05))
```


Figure 5a: Heatmap of RNA-Seq expression for genes differentially expressed between WT and KO cells (FDR<0.05). 

```{r figure5_a, warning=FALSE, cache=TRUE, echo=FALSE, comment=FALSE}
cpms <- cpm(counts[isexpr,], log=T)
my_colors = c("green", "black", "red")
my_colors = colorRampPalette(my_colors)(50)
pheatmap(cpms[row.names(deseq2_WT_vs_KO_sig),grep("KO|WT", colnames(cpms))], scale="row", fontsize=8, show_rownames=F, color = my_colors, main = "WT vs. KO, padj < 0.05")
```
